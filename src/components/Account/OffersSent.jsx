...
export const OffersSent = ({setCurrent}) => {
    ...
    const dealsData = useQuery({
        queryKey: ["dealsOffered"],
        queryFn: getQueryFnWithAuth(token, csrf, "/item/getDealsAccountSent",null ,null ,null ),
        onSuccess:(res) =>{
            setDeals(res);
        },
        onError:(err) => {
    ...
    if(dealsData.isSuccess){
        ...
        if(deals.length < 1){
            return(
                <>
                    <h1 className="w-[100%] text-2xl font-semibold font-nice">Offers Sent</h1>
        ...
        if(deals.length > 0){
            return(
                <>
                    ...
                    {deals.map((ele, indx) => <SingleDeal refetch={refetchData} sealed={false} data={ele} setCurrent={setCurrent}  key={indx}/>)}
                    </div>
                </>
            )
        }
